...
export async function prepareRagRun(opts: {
  ...
  const { embedTexts, rewriteQuery, rerankChunks } = await import("./ai.server");
  ...
  const activeDocs = (allDocs ?? []).filter(
    (d: { id: string; processing_status: string }) =>
      d.processing_status === "ready" && (!selectedSet || selectedSet.has(d.id)),
  );
  ...
  for (let qi = 0; qi < queries.length; qi++) {
    ...
    const { data: hits, error: hErr } = await supabase.rpc("hybrid_search_chunks", args);
    if (hErr) {
...
- Após chamar uma tool que gera arquivo (petition/pdf/table/presentation), confirme em UMA frase curta que o arquivo está pronto — não repita o conteúdo em texto.`;
  ...
  return { messages, tools, executor, citations, tier, model: MODEL_MAP[tier] };
}
